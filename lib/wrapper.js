...
export async function sendToQueue (queueName, options = {durable: true, persistent: true, autoDelete: false}) {
  const messageString = JSON.stringify(message)
  async function send () {
    await channel.assertQueue(queueName, options)
    await channel.sendToQueue(queueName, Buffer.alloc(messageString.length))
  }
  await handleFailedSending(send)
}
